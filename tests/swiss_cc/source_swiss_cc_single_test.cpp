...
#include "../../src/core/cpp/framework/simulation_single_swiss.h"

TEST(Swiss_cc_tests, Apply_model_single_from_file) {
    ...
    Simulation_Single_Swiss simulation;

    simulation.Read_Config(config_filenname);

    simulation.Init_parameters_filename(0);
    ...
    Analysis_Swiss  analyis = simulation.Get_analysis();

    const double MAX_RMSE_PSI_STEM = 10;

    for (const double& error: analyis.Get_rmse()) {
