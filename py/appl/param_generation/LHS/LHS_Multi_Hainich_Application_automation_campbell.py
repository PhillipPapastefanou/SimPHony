...
def Calculate_LHS_per_process(rank, ncombs, path):
    ...
    #pore_size_ind = rescale(slicer.get(), min=0.40, max = 0.49)
    camp_b = rescale(slicer.get(), min=2.5, max = 4.5)
    ...
    for i in range(ncombs):
        ...
        soil_collection[i].camp_b = camp_b[i]
    ...
    anet_max    = rescale(slicer.get(), min = 0.1, max = 5.0)
    sel_cols.append("anet_max")

    k_xylems_sats   = rescale(slicer.get(), min=0.5, max=2)
    k_xylems_sats *= KG_TO_MOL
    sel_cols.append("k_xylem_sat")

    huber_values    =  rescale(slicer.get(), min = 0.0002, max= 0.0004)
