...
class Web_Interface:
    ...
    def run_model(self,
                  k_xylem, gamma_stem, psi_leaf_50_close, gamma_leaf,
                  g0, g_stem, g1, huber,
                  root_beta, pore_size, k_soil_sat, theta_r):
        ...
        nsoil_layers = 3
        layer = SoilLayer()
        layer.k_soil_sat = k_soil_sat
        layer.psi_soil_sat = -0.5 * 1
        # layer.camp_b  = 10.4
        layer.theta_s = 0.48
        layer.theta_r = theta_r
        layer.pore_size_ind = pore_size
        ...
        params.canopy_height = 31.00
        params.huber_value = huber
        ...
        params.jackson_root_beta = root_beta
...
def update_plot():
    ...
    try:
        k_xylem_sat = float(request.form['s_k_xylem_sat'])
        gamma_stem = float(request.form['s_gamma_stem'])
        psi_leaf_50_close = float(request.form['s_psi_leaf_50_close'])
        gamma_leaf = float(request.form['s_gamma_leaf'])

        g0 = float(request.form['s_g0'])/ 1000.0
        g_stem = float(request.form['s_g_stem'])/ 1000.0
        g1 = float(request.form['s_g1'])
        huber = float(request.form['s_huber'])

        root_beta = float(request.form['s_root_beta'])
        pore_size = float(request.form['s_pore_size'])
        k_soil_sat = 10**float(request.form['s_k_soil_sat_log'])
        theta_r = float(request.form['s_theta_r'])
    ...
    fig.update_layout(
        ...
        margin = dict(l=20, r=20, t=0, b=00)
    )
