...
def create_example_hainich_parameter_list(n, filename):
    ...
    k_xylems_sats = rescale(slicer.get(), min=0.1, max=1)
    ...
    cleaf_s_log = rescale(slicer.get(), min=np.log10(0.001), max=np.log10(0.01))
...
def create_example_swiss_cc_parameter_list(n, filename):
    ...
    psi_soil_sats = rescale(slicer.get(), min=-0.1, max=-0.01)
    theta_s = rescale(slicer.get(), min=0.55, max=0.6)
    theta_r = rescale(slicer.get(), min=0.0, max=0.01)
    pore_size_ind = rescale(slicer.get(), min=0.22, max=0.3)
