...
class ParallelSetupEXCON:
    ...
    def start_simulations(self):
        ...
        for id in range(self.n_sims_per_process):
            ...
            parameters = self.parameter_parser.parameters_list[id]

            parameter_file = os.path.join(self.config.input_path, f"parameters_{id}.csv")
            ...
            Calculate_LHS_alpha(self.rank, self.ncombs_per_parameter,
                                parameters, self.alpha, parameter_file)
            ...
            self.sim.Init_Full_Parameter_Setups(np.arange(0, self.ncombs_per_parameter))
            self.sim.Init_input()
            ...
            if self.config.location == Location.Swiss_cc:
                data = np.zeros((nx, 14))
                for i in range(nx):
                ...
                df_alive = df_c.sort_values(by='alive_mean').reset_index()
                df_w = df_alive.head(1).copy()
                best_alive = df_w['alive_mean']
                best_alive_ref = 0.55
                target_rmse = best_alive/best_alive_ref

                # We are comparing against four dead trees so give the alive ones more weight
                target_rmse *= 4

                for i, best_dead_ref in zip([0, 3, 6, 7],[0.63, 0.55, 1.18, 0.97]) :
                    df_dead = df_c.sort_values(by=f't{i}').reset_index()
                    df_best_dead = df_dead.head(1).copy()

                    df_w = pd.concat([df_w, df_best_dead], axis=0)
                    best_ratio = df_best_dead[f't{i}'] / best_dead_ref
                    target_rmse +=best_ratio
