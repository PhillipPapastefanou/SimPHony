...
for id, ai in zip(selective_range, ax_indexes) :
    ...
    ax.plot(dm['psiLeaf'], zorder =1, c = 'tab:blue')
    ax.fill_between(d25.index, d25['psiLeaf'], d75['psiLeaf'], zorder =1, color = 'tab:blue' ,alpha=0.4)

    ax.set_ylim((-7.5,0))


plt.tight_layout()
plt.savefig(os.path.join(setup.config.post_path, "Best_multi_fit.png"), dpi = 150)
