...
setup.config.parameters_list_file = os.path.join(setup.config.post_path, "dummy.csv")
setup.config.config_file = os.path.join("plt", "config_dummy.txt")
# Specifying forcing and evalution data paths
setup.Export()


df = pd.read_csv(os.path.join(setup.config.post_path, "aggregated_out.csv"))
df.sort_values(by=['RMSE_MAX'],inplace=True, ascending=True)
df_top_five = df.loc[df.index[0:5].sort_values()]
df_top_five.to_csv(os.path.join(setup.config.post_path, f"parameters_best_5.csv"))
...
for id, ai in zip(selective_range, ax_indexes) :
    ...
    setup.config.parameters_list_file = os.path.join(setup.config.post_path, f"parameters_best_5.csv")
    setup.Export()
    # ----------------------------------------------------------------
    ...
    sim.Init_parameters_fn_single(ai-1)
    sim.Init_input()
