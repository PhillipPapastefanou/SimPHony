...
# Collect all dataframes (assumes each has columns like ['date', 'xylem_pressure'])
dfs = []
for i in u:
    print(i)
    df = tree_parser.trees[i].df.copy()
    dfs.append(df[['xylem_pressure']])


# Combine all trees side-by-side (columns labeled by tree id)
combined = pd.concat(dfs, axis=1)
# Compute mean, min, and max across trees per date
df_tree_mean  = combined.mean(axis=1)
df_tree_lower = combined.min(axis=1)
df_tree_upper = combined.max(axis=1)
yerr = np.vstack([
df_tree_mean - df_tree_lower,
df_tree_upper - df_tree_mean
    ])

print(yerr)
  
    
setup.config.parameters_list_file = os.path.join(post_path, f"All_RMSE_{nmax}.csv")
    
ncombs = int(pd.read_csv(setup.config.parameters_list_file).shape[0]/5)

step = int(ncombs/N_BEST)
...
for id, ai in zip(selective_range, ax_indexes):
    ...
    du =pd.read_csv(os.path.join(post_path, f"All_RMSE_{nmax}.csv"))    
    du = du.loc[du['sid'] ==id]
    du.to_csv(os.path.join(post_path, f"All_RMSE_{nmax}_{ai}.csv"))

    setup.config.parameters_list_file = os.path.join(post_path, f"All_RMSE_{nmax}_{ai}.csv")
...
labels = ['a) alive avg', 'b) dead 1', 'c) dead 2', 'd) dead 3']
...
for i, ii in zip(range(1,5),[1,2,4,5]):
    ...
    if ii == 1:
        ax.errorbar(df_tree_mean.index, df_tree_mean, yerr = yerr ,color = 'black', fmt= 'o', capsize=5, markersize=4)
    ...
    # if ii == 1:
    #     ax.set_xlim(pd.Timestamp('2018-06-01'), pd.Timestamp('2018-06-15'))
    # else:
    #     ax.set_xlim(pd.Timestamp('2018-06-01'), pd.Timestamp('2018-06-15'))
    ...
    ax.set_ylabel(r"$\psi_\mathrm{Leaf}$ [MPa]", fontsize=12)
    plt.xticks(rotation=45)
plt.subplots_adjust(hspace=0.34)
plt.savefig(os.path.join(post_path, "F1_ALL_2026.png"), bbox_inches='tight', dpi=300)
