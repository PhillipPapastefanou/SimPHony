...
var_additionals = ['psi_leaf_50_close', 'p50-p88', 'k_xylem_sat', 'stem_hydraulic_capacitance']
...
for vara in var_additionals:
    ...
    if vara == "p50-p88":
        ...
        df_rep['psi88_xylem'] =  first_row["psi50_xylem"].iloc[0] - margin * np.array([1-rf, 1, 1+rf])
        cols = cols_pos
        labels_plt = labels
    else:
        df_rep[vara] = first_row[vara].iloc[0] * np.array([1-rf, 1, 1+rf])
    ...
    for j in range(0, nrows):
        ...
        ax.plot(df_mod['psiLeaf'], color = cols[j], alpha =0.7, label = labels_plt[j])
    
    ax.tick_params(axis='x', rotation=45)
    ax.legend()
    ax.set_title(label_dict[vara])
...
plt.savefig(os.path.join(post_path, "S_F1_2.png"), bbox_inches='tight', dpi=300)
